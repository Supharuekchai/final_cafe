...
namespace final_cafe
{
    class DataAccess
    {
        ...
        public ArrayList ReturnStaffList()
        {
            ArrayList StaffList = new ArrayList();
            using (MySqlConnection connection = new MySqlConnection(ConnectionString))
            {
                MySqlCommand command = new MySqlCommand("SELECT StaffID, StaffCode, StaffName, Gender, StaffPassword, StaffLevel FROM Staffs ", connection);
                connection.Open();
                MySqlDataReader reader = command.ExecuteReader();
                ...
                if (reader.HasRows)
                {
                    while (reader.Read())
                    {
                        ...
                        string StaffCode = reader.GetString(1);
                        string StaffName = reader.GetString(2);
                        string Gender = reader.GetString(3);
                        string StaffPassword = reader.GetString(4);
                        string StaffLevel = reader.GetString(5);

                        StaffList.Add(new Details() { SalesmanID = StaffID, StaffCode = StaffCode, StaffName = StaffName, Gender = Gender, StaffPassword = StaffPassword, StaffLevel = StaffLevel });
                    }
                }
                reader.Close();
                return StaffList;
        ...
        public bool DeleteStaff(int StaffID)
        {
            try
            {
                using (MySqlConnection connection = new MySqlConnection(ConnectionString))
                {
                    MySqlCommand command = new MySqlCommand("Delete from staffs where StaffID = '" + StaffID + "'", connection);
                    connection.Open();
        ...
        public bool UpdateStaff(int StaffID, string StaffName, string StaffCode, string Gender, string StaffPassword, string StaffLevel)
        {
            using (MySqlConnection connection = new MySqlConnection(ConnectionString))
            {
                connection.Open();
                ...
                try
                {
                    ...
                    command.Parameters.AddWithValue("@StaffLevel", StaffLevel);

                    command.CommandText =
                       "Update staffs set StaffName = @StaffName, StaffCode = @StaffCode, Gender = @Gender, StaffPassword = @StaffPassword, StaffLevel = @StaffLevel where StaffID = @StaffID";
                    command.ExecuteNonQuery();
        ...
        public Details RetreiveStaffDetails(int StaffID)
        {
            Details StaffDetails = new Details();

            using (MySqlConnection connection = new MySqlConnection(ConnectionString))
            {
                MySqlCommand command = new MySqlCommand("SELECT StaffName, StaffCode, Gender, StaffPassword, StaffLevel FROM staffs where StaffID = '" + StaffID + "';", connection);
                connection.Open();
                ...
                if (reader.HasRows)
                {
                    while (reader.Read())
                    {
                        StaffDetails.StaffName = reader.GetString(0);
                        StaffDetails.StaffCode = reader.GetString(1);
                        StaffDetails.Gender = reader.GetString(2);
                        StaffDetails.StaffPassword = reader.GetString(3);
                        StaffDetails.StaffLevel = reader.GetString(4);
                    }
                }
                reader.Close();
